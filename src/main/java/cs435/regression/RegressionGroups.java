...
public class RegressionGroups {
    public static void main(String[] args) {
      ...
      VectorAssembler assembler = new VectorAssembler()
          .setInputCols(new String[]{"NumberOfGroups", "NumberOfGamesOwned",
              "TotalPlayTime"})   //number of games owned by a user
          .setOutputCol("regressionInputVector");          //set to vector for the regression input
      System.out.println("The correlation of NetWorth and NumberOfGroups is: " + gameInfo.stat()
          .corr("NetWorth", "NumberOfGroups"));
      System.out.println("The correlation of NetWorth and NumberOfGamesOwned is: " + gameInfo.stat()
          .corr("NetWorth", "NumberOfGamesOwned"));
      System.out.println("The correlation of NetWorth and TotalPlayTime is: " + gameInfo.stat()
          .corr("NetWorth", "TotalPlayTime"));

      Dataset<Row> vectorData = assembler.transform(gameInfo);
      vectorData.show();    //testing output to make sure we got here right

      LinearRegression lr = new LinearRegression().setLabelCol("NetWorth")
          .setFeaturesCol("regressionInputVector");
